...
def main():
    ...
    # Writing readme
    readme_f = '%s/%s.readme' % (args.root_dir, args.simu_id)
    ...
    omega = np.random.uniform(size = (args.num_tasks, args.num_tasks))
    omega = omega.transpose().dot(omega)
    d = np.diag(omega)
    d.shape = (args.num_tasks, 1)
    omega = omega / np.sqrt(d.dot(d.transpose()))

    # Save omega to file
    fname = "%s/%s.task_similarities.txt" % (args.root_dir, args.simu_id)
    ...
    b = np.random.normal(size=(args.num_tasks, NUM_CAUSAL_TOTAL))
    beta = L.dot(b)
    ...
    fname = "%s/%s.causal_features.txt"  % (args.root_dir, args.simu_id)
    np.savetxt(fname, causal_features, fmt='%d')
    logging.info("Causal features saved under %s\n" % fname)

    # Save beta to file
    fname = "%s/%s.causal_weights.txt"  % (args.root_dir, args.simu_id)
    ...
    fname = "%s/%s.genotypes.txt"  % (args.root_dir, args.simu_id)
    with tb.open_file(fname, 'w') as h5f:
        filters = tb.Filters(complevel=5, complib='blosc')
        Xtr = h5f.create_carray(h5f.root, 'Xtr', tb.Int8Atom(),
                                shape=(args.num_features, args.num_samples))
        for row in xrange(args.num_features):
            Xtr[row, :] = np.random.random_integers(0, high=2, size=args.num_samples)
        h5f.close()
    ...
    with tb.open_file(fname, 'r') as h5f:
        ...
        for task_idx in range(args.num_tasks):
            ...
            y += np.random.normal(scale=0.1, size=(args.num_samples, ))
            fname = "%s/%s.phenotype_%d.txt" % (args.root_dir, args.simu_id, task_idx)
            ...
            fname = "%s/%s.scores_%d.txt" % (args.root_dir, args.simu_id, task_idx)
    ...
    num_modules = args.num_features / MOD_SIZE
    num_edges = MOD_SIZE * (MOD_SIZE - 1) * num_modules + \
                2 * (num_modules - 1) + 2 * (args.num_features - MOD_SIZE * num_modules)
    dimacs_f = '%s/%s.network.dimacs' % (args.root_dir, args.simu_id)
    with open(dimacs_f, 'w') as g:
        g.write("p max %d %d\n" % ((args.num_features+2), num_edges))
        # g.write("n %d s\n" % (args.num_features+1))
        # g.write("n %d t\n" % (args.num_features+2))
        ...
        for mod_idx in range(num_modules):
            ...
            if (x+MOD_SIZE) < args.num_features: # connect to next module
                g.write("a %d %d 1\n" % ((x+MOD_SIZE), (x+MOD_SIZE+1)))
        # connect each of the remaining nodes to its neighbor
        for x_idx in range(x+MOD_SIZE+1, args.num_features):
            g.write("a %d %d 1\n" % ((x_idx), (x_idx-1)))
            g.write("a %d %d 1\n" % ((x_idx), (x_idx+1)))

...
if __name__ == "__main__":
    ...
    # xp_indices for all folds : 
    # TODO : factorisation of fname template...
    trIndices_fname = args.data_dir+'/'+args.simu_id+'.fold%d.trIndices'
    teIndices_fname = args.data_dir+'/'+args.simu_id+'.fold%d.teIndices'
    ssIndices_fname = args.data_dir+'/'+args.simu_id+'.fold%d.ss%d.ssIndices'

    for fold_idx in args.num_folds : 

        with open(trIndices_fname %(fold_idx), 'r') as trIndices_f : 
            xp_indices[fold_idx]["trIndices"] = trIndices_f.readline().split()
        with open(teIndices_fname %(fold_idx),'r') as teIndices_f : 
            xp_indices[fold_idx]["teIndices"] = teIndices_f.readline().split()
        for ss_idx in xrange(args.num_subsamples) :
            with open(ssIndices_fname  %(fold_idx,ss_idx), 'r') as ssIndices_f:
                xp_indices[fold_idx]["ssIndices"].append( ssIndices_f.readline().split() ) 

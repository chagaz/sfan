...
namespace dynamic{
	...
	*/
	...
	public:
		...
		void resize(int Size, const type & val = type()){
			if(!owned)
				std::cout << "cant resize referenced buffer" << std::endl;
			if(Size<=capacity()){
				if(Size>size()){
	...
	public:
		...
		void pop_back(){
			if(!empty()){
				...
				std::cout << "pop from empty buffer occured" << std::endl;
			};
		};
	...
	*/
	...
	public:
		...
		type & back(){
			if(this->is_empty())
				std::cout << "memmanager::queue is empty" << std::endl;
	...
	*/
	...
	public:
		...
		void add(const type & x){
			if(is_full()){
				std::cout << "buffer cant hold more elements" << std::endl;
			};
			(*this)[i_end++] = x;
		};
		void push_back(const type & x){
		...
		void reserve(int Size){
			if(Size<this->capacity())
				std::cout << "buffer capacity is fixed" << std::endl;
		};
		...
		void resize(int n){
			if(n>this->capacity())
				std::cout << "buffer capacity is less then requested" << std::endl;
			i_end=n;
		};
	};

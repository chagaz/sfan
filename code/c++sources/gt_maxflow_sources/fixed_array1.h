	...
	*/
	...
	public:
		...
		void operator = (const fixed_array1 & x){
			...
			if(!allocated()){
				...
				if(capacity()<xs)
					std::cout << "reallocation of fixed_array1 is not allowed" << std::endl;
			};
			if(begin()+xs<end()){
	...
	public:
		...
		type * push_back(const type & val){
			if(free_capacity()==0)
				std::cout << "fixed_array1 capacity exceeded" << std::endl;
			create(end(),val);
			type * r = _end;
			++_end;
			return r;
		...
		void pop_back(){
			if(empty())
				std::cout << "pop_back from empty fixed_array1" << std::endl;
			destroy(end());
			--_end;
		};
		...
		void erase(type * it){
			if(!(it>begin() && it<end()))
				std::cout << "erase out of range of fixed_array1" << std::endl;
			destroy(it);

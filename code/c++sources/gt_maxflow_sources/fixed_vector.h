	...
	*/
	...
	public:
		...
		void operator = (const fixed_vector & x){
			...
			if(!allocated()){
				...
				if(capacity()<x.size())
					std::cout << "reallocation of fixed_vector is not allowed" << std::endl;
			};
			if(begin()+x.size()<end()){
	...
	public:
		...
		void push_back(const type & val){
			if(free_capacity()==0)
				std::cout << "fixed_vector capacity exceeded" << std::endl;
			create(end(),val);
			++_end;
		};
		...
		void pop_back(){
			if(empty())
				std::cout << "pop_back from empty fixed_vector" << std::endl;
			destroy(end());
			--_end;
		};
		...
		void erase(type * it){
			if(!(it>begin() && it<end()))
				std::cout << "erase out of range of fixed_vector" << std::endl;
			destroy(it);

...
template<typename tcap> void hi_pr::construct(int _nV, int _nE, const int * _E, const tcap * _cap, const tcap * _excess){
	...
	float   **cap_ad=&cap;         /* address of the array of capacities */
...
#define EN22 20
	...
	for(int e=0;e<_nE;++e){
		tail = _E[2*e];
		head = _E[2*e+1];
		double cap1 = _cap[2*e];
		double cap2 = _cap[2*e+1];
...
void inline hi_pr::add_arc(int tail, int head, float cap1, float cap2, long *& arc_first, long *& arc_tail, long & pos_current, arc *& arc_current, node *& nodes, long & node_min, long & node_max, long & no_alines){
